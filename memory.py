...
async def get_signal_result(redis, signal_id: str): """ Read result written by execution bot """ key = f"signal:result:{signal_id}"
...
async def should_block_signal( redis, symbol: str, direction: str, market_regime: str, htf_trend: str ) -> bool: """ If same conditions failed >= 2 times in last 3 → block signal """
...
async def debug_signal_memory(redis, symbol: str): """ Print last signals for inspection """ history = await redis.lrange(f"signal:history:{symbol}", 0, 9)
